...
export class StapeldammenGame extends GameBase {
    ...
    public collectChatLogLine(lines: ChatLogLine[], r: APMoveResult, ctx: ChatLogCollectContext): boolean {
        switch (r.type) {
            case "add":
                this.pushSeatChatLine(lines, ctx.defaultSeat, "apresults:ADD.add", {where: r.where!, count: r.num!});
                return true;
            case "capture":
    ...
    public static clone(obj: StapeldammenGame): StapeldammenGame {
        const cloned = Object.assign(new StapeldammenGame(), cloneState(obj) as StapeldammenGame);
        return cloned;
    }
