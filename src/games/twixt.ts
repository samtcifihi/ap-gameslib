...
import { reviver, UserFacingError, generateColumnLabel } from "../common/index.js";
...
export class TwixtGame extends GameBase {
    public static readonly gameinfo: APGamesInformation = {
        ...
        categories: ["goal>connect", "mechanic>place", "mechanic>block", "board>shape>rect", "board>connect>rect", "components>special", "components>simple>pnp"],
        flags: ["pie"],
    ...
    public collectChatLogLine(lines: ChatLogLine[], r: APMoveResult, ctx: ChatLogCollectContext): boolean {
        switch (r.type) {
            case "place":
                this.pushSeatChatLine(lines, ctx.defaultSeat, "apresults:PLACE.twixt", { where: r.where! });
                return true;
            case "add":
                this.pushSeatChatLine(lines, ctx.defaultSeat, "apresults:ADD.twixt", { where: r.where! });
                return true;
            case "remove":
                this.pushSeatChatLine(lines, ctx.defaultSeat, "apresults:REMOVE.twixt", { where: r.where! });
                return true;
            default:
