...
import { reviver, UserFacingError, SquareGraph } from "../common/index.js";
...
export class VirusWarGame extends GameBase {
    ...
    public validateMove(m: string): IValidationResult {
        ...
        if (m.length === 0) {
            ...
            result.message = i18next.t("apgames:validation.viruswar.INITIAL_INSTRUCTIONS_other", {count: this.numMoves})
            return result;
        }
        ...
        } else if ( this.numMoves - moves.length === 1 ) {
            result.message = i18next.t("apgames:validation.viruswar.INITIAL_INSTRUCTIONS_one");
        } else {
    ...
    public collectChatLogLine(lines: ChatLogLine[], r: APMoveResult, ctx: ChatLogCollectContext): boolean {
        switch (r.type) {
            case "place":
                this.pushSeatChatLine(lines, ctx.defaultSeat, "apresults:PLACE.viruswar", { where: r.where! });
                return true;
            case "eog":
