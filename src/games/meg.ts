...
export class MegGame extends GameBase {
    public static readonly gameinfo: APGamesInformation = {
        ...
        categories: ["goal>arrange", "mechanic>move", "mechanic>place", "mechanic>asymmetry", "mechanic>differentiate", "board>shape>rect", "board>connect>rect", "components>simple>1c"],
        flags: []
    };
    ...
    public collectChatLogLine(lines: ChatLogLine[], r: APMoveResult, ctx: ChatLogCollectContext): boolean {
        switch (r.type) {
            case "place":
                this.pushSeatChatLine(lines, ctx.defaultSeat, "apresults:PLACE.meg", {context: r.what!, where: r.where!});
                return true;
            case "claim":
                this.pushSeatChatLine(lines, ctx.defaultSeat, "apresults:CLAIM.meg", {where: r.where!});
                return true;
            case "move":
                this.pushSeatChatLine(lines, ctx.defaultSeat, "apresults:MOVE.meg", {from: r.from!, to: r.to!});
                return true;
            default:
