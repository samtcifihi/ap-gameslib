...
export class SpireGame extends GameBase {
    public static readonly gameinfo: APGamesInformation = {
        ...
        categories: ["goal>immobilize", "mechanic>place", "mechanic>share", "board>shape>rect", "board>connect>rect", "components>simple>3c", "components>shibumi", "board>3d"],
        flags: [],
        displays: [{ uid: "orb-3d" }],
    };
    ...
    public collectChatLogLine(lines: ChatLogLine[], r: APMoveResult, ctx: ChatLogCollectContext): boolean {
        switch (r.type) {
            case "place":
                if (r.who === 3) {
                this.pushSeatChatLine(lines, ctx.defaultSeat, "apresults:PLACE.spire_neutral", { where: r.where! });
                } else {
                this.pushSeatChatLine(lines, ctx.defaultSeat, "apresults:PLACE.spire_player", { where: r.where! });
                }
                return true;
            default:
