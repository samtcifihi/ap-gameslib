...
import { Direction, RectGrid, reviver, UserFacingError } from "../common/index.js";
...
export class ResolveGame extends GameBase {
    public static readonly gameinfo: APGamesInformation = {
        ...
        categories: ["goal>connect", "mechanic>place", "mechanic>move", "mechanic>displace", "board>shape>rect", "board>connect>rect", "components>simple>1per"],
        flags: ["pie"],
    };
    ...
    public collectChatLogLine(lines: ChatLogLine[], r: APMoveResult, ctx: ChatLogCollectContext): boolean {
        switch (r.type) {
            case "place":
                ...
                return true;
            case "swap":
                this.pushSeatChatLine(lines, ctx.defaultSeat, "apresults:SWAP.resolve", { where: r.where! });
                return true;
            case "select":
                this.pushSeatChatLine(lines, ctx.defaultSeat, "apresults:SELECT.resolve", { where: r.where! });
                return true;
            default:
