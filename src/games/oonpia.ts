...
export class OonpiaGame extends GameBase {
    public static readonly gameinfo: APGamesInformation = {
        ...
        categories: ["goal>annihilate", "mechanic>place", "mechanic>capture", "mechanic>enclose", "board>shape>hex", "board>connect>hex", "components>simple>2per"],
        variants: [
    ...
    public collectChatLogLine(lines: ChatLogLine[], r: APMoveResult, ctx: ChatLogCollectContext): boolean {
        switch (r.type) {
            case "place":
                this.pushSeatChatLine(lines, ctx.defaultSeat, "apresults:PLACE.oonpia", {where: r.where!, what: r.what!});
                return true;
            case "capture":
                this.pushSeatChatLine(lines, ctx.defaultSeat, "apresults:CAPTURE.oonpia", {count: r.count!, what: r.what!});
                return true;
            default:
